...
void retrievAndShutdown() {
    ...
    for (i=0; i<nimage; i++) {
        ...
        for(iLen = 1;iLen<=len;++iLen){
            ...
            mi = startO[iCache][iLen];
            mm = shiftmax(i, mi, mx, my, rm);
            basisResp[iLen*2-1+i*(2*len+1)]=mm;
            /* draw(Asym[i], mx, my, mi, sqrt(imaxi[i*5])); */
            draw(Asym[i], rm[1], rm[2], rm[3], mm/6.0f);
            inhibit(i, rm[3], rm[1], rm[2]);
...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    basisResp = mxGetPr(prhs[c++]); /*retrive the individual basis response*/
    ...
    storeShift();
    curveShift();
    /*warninig assume only one image*/
    retrievAndShutdown();
