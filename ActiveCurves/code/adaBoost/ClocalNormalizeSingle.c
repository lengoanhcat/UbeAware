...
{            
 ...
 /* variables */
float **SUM1map, **SUM1mapAll, **integralMap, **averageMap, thresholdFactor;    
...
void LocalNormalize()
{
   ...
   SUM1mapAll = single_matrix(sizex, sizey); 
      for (x=0; x<sizex; x++)
       for (y=0; y<sizey; y++){
    ...
    integralMap = single_matrix(sizex, sizey);
    ...
    averageMap = single_matrix(sizex, sizey); 
    ...
    for (k=0; k<8; k++) 
     ...
     for (x=0; x<sizex; x++)
       ...
       {      
		...
		averageDivide = 1.0/(averageDivide+1e-10);
        for (orient=0; orient<numOrient; orient++)
